...
class CRUDRepository(CRUDBase[Repository, RepositoryCreate, RepositoryUpdate]):
    ...
    def get_all_topics(self, db: Session, limit: int = 100) -> List[Any]:
        from sqlalchemy import func

        # This query unnests the topics array, groups by topic, counts them,
        # orders by count descending, and limits the result.
        topic_col = func.unnest(self.model.topics).label("topic")
        query = (
            db.query(topic_col, func.count(1).label("count"))
            .group_by(topic_col)
