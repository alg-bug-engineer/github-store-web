...
class GitHubAPIClient:
    ...
    def _check_rate_limit(self):
        ...
        if self.rate_limit_remaining < 50:
            ...
            try:
                ...
                logger.info(
                    f"GitHub API Rate Limit: {self.rate_limit_remaining} remaining, resets at {datetime.fromtimestamp(self.rate_limit_reset_time)}"
                )
        ...
        if self.rate_limit_remaining <= 5:
            ...
            if sleep_time > 0:
                logger.warning(
                    f"GitHub API rate limit almost exhausted. Sleeping for {math.ceil(sleep_time / 60)} minutes."
                )
    ...
    def _make_request_no_rate_check(
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and "rate limit exceeded" in e.response.text.lower()
            ):
        ...
        while True:
            ...
            try:
                ...
                if (
                    ...
                    logger.error(
                        "GitHub API rate limit exceeded during pagination. Pausing for 30 minutes."
                    )
                    time.sleep(30 * 60)
                    self.rate_limit_remaining = -1
                else:
                ...
                logger.error(
                    f"A network error occurred while fetching releases for {owner}/{repo}: {e}"
                )
                break
