...
def _sync_single_repository(
    github_item: Dict[str, Any], initial_category: Optional[str] = None
):
    ...
    try:
        ...
        logger.error(f"An error occurred during the sync of {github_item.get('full_name')}. Rolling back.", exc_info=True)
        db.rollback()
    else:
        db.commit()
    finally:
...
def sync_all_github_data():
        ...
        try:
            ...
            for page_num in range(1, 3):  # Fetch first 2 pages
                ...
                for item in search_results.get("items", []):
                    _sync_single_repository(item, initial_category=category_name)
        except Exception as e:
            logger.error(
    ...
    for db_repo in all_db_repos:
        try:
            ...
            _sync_single_repository(github_item, initial_category=db_repo.category)
        except Exception as e:
            logger.error(
